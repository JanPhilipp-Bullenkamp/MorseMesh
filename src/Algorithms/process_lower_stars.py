...
#
# @brief Contains the process_lower_stars function described in 
# Robins et al. (DOI: 10.1109/TPAMI.2011.95)
...
def lower_star(vertex, edges_dict: dict, faces_dict: dict):
    """! @brief Extracts the lower star of a vertex.
    @details Contains the vertex and all edges and faces with lower function 
    values than the vertex.
    ...
    @return lower_star A dictionary containing the keys 'vertex', 
            'edges' and 'faces' which each contain key value pairs 
            of index and edge/face (Simplex class objects). The edges 
            and faces are sorted by function values.
    """
    # x a vertex
    ...
    lower_star['edges'] = {k: v for k, v in sorted(lower_star['edges'].items(), 
                                                   key=lambda item: item[1].fun_val)[::-1]}
    lower_star['faces'] = {k: v for k, v in sorted(lower_star['faces'].items(), 
                                                   key=lambda item: item[1].fun_val)[::-1]}
    return lower_star

def num_unpaired_faces(face, PQzero: PriorityQueue):
    """! @brief Gives the number of unpaired faces of a face, so the number 
    of edges of a face that are not paired.
    
    @param face The face we want to get the number of unpaired faces of.
    @param PQzero The Priority Queue with 0 unpaired faces.
    
    @return number Returns the number of faces (edges) of a face 
            (triangle) that have not been paired.
    """
    # checks number of faces in PQzero ( should be equal to number of 
...
def pair(face, PQzero: PriorityQueue):
    """! @brief Returns a pair of face and edge.
    
    @param face The face that should be paired (which has exactly one unpaired face).
    @param PQzero The Priority Queue with 0 unpaired faces.
    
    @return index, simplex A tuple of the edge index that is paired 
            with this face and the edge Simplex object.
    """
...
def process_lower_stars(vertices_dict: dict, 
                        edges_dict: dict, 
                        faces_dict: dict, 
                        C: dict, 
                        V12: dict, 
                        V23: dict):
    """! @brief The function described in Robins et al. 2011, that returns 
    a discrete gradient vector field.
    
    @details Loops over all vertices, calculates their lower stars and 
    pairs up as many edge-vertex (V12) or face-edge (V23) pairs as 
    possible. Simplices that cannot be paired will become critical simplices and 
    are stored in C.
            ...
            while (PQone.notEmpty() or PQzero.notEmpty()):
                while PQone.notEmpty():
                    ...
                    if num_unpaired_faces(alpha_simplex, PQzero) == 0:
                        ...
                        for Findex, face in lowerStar['faces'].items():
                            if (num_unpaired_faces(face, PQzero) == 1 
                                and (face.has_face(alpha_simplex) 
                                     or face.has_face(pair_simplex))):
                                PQone.insert(tuple((face, Findex)))
                ...
                if PQzero.notEmpty():
                    ...
                    for Findex, face in lowerStar['faces'].items():
                        if (num_unpaired_faces(face, PQzero) == 1 
                            and face.has_face(gamma_simplex)):
